    ...
    def driver(self):
        if self._driver is None:
            ...
            if Cfg.Browser.get('remote_url'):
                self._driver = Remote( # pragma: no cover
...
class Elements(Element):
    ...
    def wait_for_item_with_text_to_load(self, item_text, timeout_seconds=2):
        def wait_by_text():
            Browser().recreate_session()
            return self.get_by_text(text=item_text)

        return wait(wait_by_text,
             timeout_seconds=timeout_seconds,
             waiting_for=f'{item_text} in items')

    def wait_for_items_load(self, items_count, timeout_seconds=2):
        def check_count():
            if not items_count:
                Browser().driver.implicitly_wait(0) # pragma: no cover
